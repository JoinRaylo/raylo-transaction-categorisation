"""Bounded private candidate draw from the customer-linked Plaid pool.

This is a source draw for admission engineering, not a benchmark reservation or
label export. It uses one deterministic hash-ranked SELECT, keeps source and
identity fields private, and leaves provider categories out of the annotation
payload projection. A later admission step must still verify aliases, exposure,
families, blocks and authority state.
"""
...
def main() -> None:
    ...
    parser.add_argument("--candidate-limit", type=int, default=5_000)
    parser.add_argument("--seed", default="b05-pilot-source-v1")
    ...
    sql = Path(__file__).with_name("benchmark_candidate_extract.sql").read_text()
    ...
    receipt = {
        ...
        "sql_sha256": digest(Path(__file__).with_name("benchmark_candidate_extract.sql")),
    ...
    if args.execute:
        ...
        job = client.query(
            sql, job_config=config, location=LOCATION, job_id_prefix="txncat_candidate_"
        )
        ...
        print(f"Private candidate draw complete: {count:,} rows; no admission authorized.")
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(
            "Candidate extraction failed; private output is incomplete and not admissible."
        ) from None
