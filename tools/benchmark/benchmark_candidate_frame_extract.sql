-- Deterministic G2 candidate-frame draw from the current customer-linked Plaid
-- materialization.  Extends benchmark_candidate_extract.sql with the provider
-- category fields and a content-revision digest required by the candidate
-- frame contract.  The output is private engineering input; it is not an
