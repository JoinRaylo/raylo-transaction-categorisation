...
- **B03-B physical scope review (17 Sep):** [Scope and proof checklist](docs/benchmark-implementation/b03-b-physical-scope-review-2026-09-17/README.md). The historical partial-scope packet records the earlier `raylo-production` synthetic fixture decision; it is not the current authority boundary. The current resource record is [B03-B authority resource provisioning](docs/benchmark-implementation/b03-b-authority-resource-provisioning-2026-09-17/README.md). No customer data, membership, labels or B04 consumer were accessed.

- **B03-B authority resource provisioning (17 Sep):** [Current resource and access evidence](docs/benchmark-implementation/b03-b-authority-resource-provisioning-2026-09-17/README.md). The approved `raylo-txncat-authority-prod` project is linked to the same billing account as `raylo-production`; its empty EU bucket, HSM keys and protected Google-managed-encryption Firestore registry are verified. The parent folder's inherited `team-infra-eng` Owner grant and unreadable organisation policy mean strict effective isolation is not certified, but that risk is accepted for the internal benchmark. The follow-on runtime proof packet records the named identities, exact bindings and synthetic live proof; no real data or customer-linked objects were touched.

- **B03-B runtime identities and synthetic live proof (18 Sep):** [Runtime proof packet](docs/benchmark-implementation/b03-b-runtime-identities-2026-09-18/README.md). Four keyless service accounts, scoped custom IAM, the private verifier and the HSM receipt-key lifecycle passed a bounded synthetic v3 live matrix: a barrier-synchronised Firestore CAS race reached the registry and produced one `committed` and one `stale_epoch`, valid worker requests returned 200, pending/orphan/changed-reference/cross-plane/contaminated/wrong-audience requests were rejected, direct worker/verifier data access was denied, and disabling the receipt key failed closed before recovery. The v3 manifest is explicitly non-authorizing and records its generation, digest and ownership/execution receipt; writer-hardening, forged/unknown-subject and key-rotation cases remain deferred. Only synthetic objects were written, and fresh linked-only candidate admission remains the next gate.
...
- **B03-C private pilot annotation and review queue (18 Sep):** [Completion evidence](docs/benchmark-implementation/b03-c-private-pilot-annotation-2026-09-18/README.md). All three independent batch streams now contain 500/500 strict-valid votes. Two gated Gemini attempt-3 recoveries restored the final 26 and 14 rows after 10/10 synthetic acceptance suites. Comparison found 239 model disagreements and 24 unanimous abstentions; the corrected owner-only v2 workbook exposes all 263 unresolved cases for Carlos without source IDs or automatic adjudication. No final labels, training, scoring or cloud mutation occurred.

- **B03-C ID-bearing training membership (18 Sep):** [Implementation and evidence](docs/benchmark-implementation/b03-c-id-bearing-training-membership-2026-09-18/README.md). Future Tier-B fetches now retain exact Plaid account/transaction IDs from the approved customer-linked pool and write a separate private train/selection lookup without changing model JSONL. Legacy rows without IDs are reported as unresolved, and cross-role or changed-payload identity reuse fails closed. Permanent assignments survive rebuilds and a final coverage marker binds the lookup to both model files. The query compiled in a no-row BigQuery dry run; 89 research tests and 101 canonical B02/B03 regressions pass. Independent Astra review is approved with no remaining findings. No real row, label, provider call, retrain or locked-set score occurred.

- **B03-E private evaluation pilot (18 Sep):** [Construction evidence](docs/benchmark-implementation/b03-e-private-eval-pilot-2026-09-18/README.md). The receipt-bound 5,000-row linked-Plaid draw now has a private 500-event eval lookup and separate identifier-free annotation payload: 250 representative, 150 unseen-input and 100 unfamiliar-merchant primary rows across 464 whole customer blocks. Strict novelty cohorts have zero known historical input matches; the representative cohort intentionally includes 111 familiar inputs. Future Tier-B fetch/build now require this lookup and exclude exact event/account/customer groups under a hash-bound receipt. Twenty-four focused protection tests and 135 regressions pass; Astra approved both construction and protection after corrections. No provider call, label, real training fetch, retrain, locked-set score or cloud mutation occurred.

- **B03-D synthetic candidate allocation (17 Sep):** [Design and evidence](docs/benchmark-implementation/b03-d-synthetic-allocation-2026-09-17/README.md). A strict synthetic-fixture/policy-scoped planner now assigns whole opaque identity/projection blocks in protected-before-training order, preserves the three-view semantics, rejects strict novelty collisions and contradictory blocks, and records explicit capacity shortfalls without authority. 19 adversarial allocator tests plus the full library/app unit suites pass. No real rows, labels, authority writes, locked-set access, scoring or retraining occurred; the retained runtime permission proof, fresh linked-only admission and managed reservation are the next real-data gates.
- **G0 synthetic two-cohort contracts (21 Sep):** [Design and evidence](docs/benchmark-implementation/g0-synthetic-contracts-2026-09-21/README.md). The 2,000-row benchmark target is now pinned as an unchanged 1,500-row core (500 pilot + 1,000 new rows; the 70/20/10 view split applies to the core only) plus a separately reported 500-row rare-leaf supplement marked `cohort=rare_leaf_supplement`. The canonical app modules `benchmark_cohort.py` and `benchmark_lifecycle.py` provide hash-bound cohort membership, frozen-design allocation verification, an eight-state no-skip phase ledger, an append-only receipt chain and a nine-rule conflict matrix, with backward-compatible annotation additions for cancelled attempts and bounded retry exhaustion. The contracts enforce pilot immutability, union protection that fails closed, headline exclusion of the supplement, and a pre-label proxy selector that rejects candidates carrying final labels. 76 focused and 1,291 library unit tests pass. No real row was read, reserved or labelled, no provider call or cloud change occurred, no retraining or locked-set scoring happened, and the phase ledger's real state remains synthetic-only; the next gate is G1 pilot closure (AIE-511/AIE-512/AIE-513 evidence).

- **B03-F pilot reconstruction and Opus 5 adjudication (21 Sep):** [Reconstruction and adjudication evidence](docs/benchmark-implementation/b03-f-pilot-reconstruction-adjudication-2026-09-21/README.md). The lost v2 pilot was recovered byte-identically: the identity re-derivation returned all 500 rows (474 accounts, 464 customers), the B02 exposure index rebuilt to a byte-identical `database_sha256` `61cc0a42…` under the preserved key, and the re-run deterministic allocation reproduced `pilot.jsonl` (`fc5c626c…`) and `membership.csv` (`0afb4155…`) exactly, with every view fingerprint passing. Provider recovery yielded 500/499/499 strict-valid votes (two unretried Gemini `MAX_TOKENS` items accepted as incomplete; retry cap respected). A `claude-opus-5` batch-only first pass produced 265/265 strict-valid proposals (156 labelled / 83 ambiguous / 26 insufficient evidence) that are explicitly not independent votes; the lead review agreed 204, disagreed 33, was unsure 28, and escalated 61 rows plus six policy groups to Carlos. `carlos_decisions=0`, `final_labels_created=0`, `authorizes_consumption=false`; the next gate is Carlos's rulings, then the AIE-512 importer.

- **B03-B effective IAM gate diagnosis (17 Sep):** [Read-only resolution packet](docs/benchmark-implementation/b03-b-effective-iam-gate-resolution-2026-09-17/README.md). Carlos's direct project Owner grant is present, but the project inherits `team-infra-eng` Owner from the shared `internal-services-monorepo` folder; organization IAM, nested group membership, deny/PAB and impersonation paths are not certified. The recommended resolution is placement under a dedicated clean folder with governed/JIT administration. No shared-folder change, IAM mutation, API enablement, identity, fixture, BigQuery read or customer data access occurred.

- **B03-B internal-benchmark risk acceptance (17 Sep):** Carlos accepted the inherited control-plane risk for this internal, versioned benchmark, so strict clean-folder/effective-IAM attestation is no longer a blocker. Permanent train/selection/evaluation separation, novelty/group protections, named least-privilege runtime paths, immutable/versioned artifacts, customer-linked Plaid-only scope and the closed anonymous-ID boundary remain mandatory. Future benchmark expansion or production promotion requires the risk decision to be revisited.

- **B03-B fresh linked-only admission profile (17 Sep):** [Aggregate evidence](docs/benchmark-implementation/b03-b-linked-admission-profile-2026-09-17/README.md). A fresh EU `SELECT` profiled 36,706,094 materialized Plaid rows and 20,653,747 existing customer-linked observations across 46,273 customers; 16,052,347 rows remain excluded by the link rule. The profile is aggregate-only and does not establish candidate-level alias/history, training/selection exposure, view-specific novelty, block sizes or pilot eligibility. No rows, identifiers, labels or authority objects were exported.

- **B03-B candidate event-lineage evidence (18 Sep):** [Private aggregate evidence](docs/benchmark-implementation/b03-b-candidate-lineage-2026-09-18/README.md) binds a 5,000-event linked-only candidate draw to 5,636 selected-lineage rows under a 20 GB BigQuery cap. It confirms candidate block counts and exposes pending/history/family limitations, but the available sources do not expose `pending_transaction_id` or complete raw-account history. Eligible rows remain 0; no candidate was admitted, reserved, labelled or sent to a provider.
