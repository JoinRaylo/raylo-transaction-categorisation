...
- **B03-B runtime identities and synthetic live proof (18 Sep):** [Runtime proof packet](docs/benchmark-implementation/b03-b-runtime-identities-2026-09-18/README.md). Four keyless service accounts, scoped custom IAM, the private verifier and the HSM receipt-key lifecycle passed the synthetic Stage 0–3 proof: real Firestore CAS produced one `committed` and one `stale_epoch`, valid worker requests returned 200, cross-plane/contaminated/wrong-audience requests were rejected, direct worker/verifier data access was denied, and disabling the receipt key failed closed before recovery. The v2 manifest is explicitly non-authorizing; only synthetic objects were written, and fresh linked-only candidate admission remains the next gate.
