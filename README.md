...
- **B03-B physical scope review (17 Sep):** [Scope and proof checklist](docs/benchmark-implementation/b03-b-physical-scope-review-2026-09-17/README.md). The approved partial setup now contains the dedicated EU bucket with its HSM storage CMEK attached, keyring and HSM keys. The registry design conditionally uses a dedicated Google-managed-encryption Firestore database because CMEK is not required by the protocol; effective policy allows non-CMEK services, but explicit security/data-governance owner approval remains pending. Service identities, IAM bindings and live proofs remain pending. No customer data, membership, labels or B04 consumer were accessed.
