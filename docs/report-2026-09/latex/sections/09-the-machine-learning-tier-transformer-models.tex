...
\end{tikzpicture}}
\caption*{The sentence format gives the model what the linear classifier sees plus amount and direction. The mask is applied after the heads, so an impossible category can never win. The scores and the attention description are illustrative of the mechanism rather than an inspected prediction. At serving time the whole path runs on CPU at about 360 transactions a second.}
\end{figure}

What this means in practice: the linear classifier recognises character patterns, so it can place a new merchant only when its name happens to share fragments with ones already labelled. The transformer has read millions of narratives and has learned what the name of a bookmaker or a lender tends to look like in a bank statement, so it gets more of them right before any label exists. That is exactly where the pipeline's remaining errors sit, which is why its gain is concentrated on never-seen merchants and on the high-risk categories.
...
Eight iterations were run over four days, each against pre-declared criteria and, once a recipe stabilised, with three seeds. The early ones fixed bugs and showed that the transformer's first apparent lead on credits was a data effect: given the same credit tranche, the linear model caught up. What did not go away was its lead on merchants it had never seen and on general-level accuracy. Three levers were then pulled, each acting on a different limitation.

\begin{enumerate}
\item \textbf{More labelled data} where the evaluation sets said the gap was: credits, then risk debits. Both models received it and both gained equally (section 8).
\item \textbf{A larger encoder with full pretraining.} BERT-small (29 million parameters, pretrained on 3 million sentences on a laptop) was replaced by DistilBERT (66 million parameters, pretrained on the full 21.5 million-sentence corpus on a rented GPU for about seven pounds). The gain was one point on generic novel strings but four to five points on the risk set and three to four more at general level, so the larger encoder was kept.
\item \textbf{Distillation from the labelling committee.} The encoder's first training pass used the 405,000 texts where Gemini and Sonnet agreed, in place of labels derived from our own rules. The committee's conventions on common texts, which the linear model also memorises, are carried by the encoder to texts it has never seen. This added a further one to two points on every cut.
\end{enumerate}
...
\end{figure}
Every accuracy criterion set at the start now clears with confidence intervals excluding zero. The remaining cost is speed: the transformer scores at about a third of the linear model's rate. At 360 rows a second the 42\% of Plaid rows that reach the classifier take about 85 CPU-minutes, or about 200 minutes if every row in the 4.3 million-row table were scored. That has been judged acceptable for now. Quantisation would recover a factor of two to three if it becomes a constraint.

The full-pipeline number moved by only one point, because 76\% of transactions never reach the classifier. The transformer's value is concentrated where the rules cannot help: merchants nobody has labelled, and the high-risk categories the credit model cares about most.
