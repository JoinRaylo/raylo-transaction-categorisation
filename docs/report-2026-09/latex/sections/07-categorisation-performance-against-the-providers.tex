...
\caption*{Leaf-level accuracy on the same rows. Rows with no provider category are counted as wrong; on provider-labelled rows alone the status quo reaches 45\%. At general level the rules-plus-classifier pipeline is 87\% and the status quo 42\%.}
\end{figure}
...
\caption*{Leaf-level accuracy on the rows that reach the machine-learning tier.}
\end{figure}
