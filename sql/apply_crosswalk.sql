...
eqx_resolved AS (
  SELECT r.*,
    CASE
      ...
      WHEN LOWER(TRIM(r.vendor))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'repayment') THEN 'loan_repayment_manual'
      WHEN LOWER(TRIM(r.vendor))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.vendor))='fitzsimons' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.vendor))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'loan\s+repayment|repayment') THEN 'loan_repayment_manual'
      WHEN LOWER(TRIM(r.vendor))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.vendor))='jasmine' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.vendor))='hart' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.vendor))='blackwell\'s' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.vendor))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'bibs|clothing') THEN 'clothing_general'
      WHEN LOWER(TRIM(r.vendor))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.vendor))='pier' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '(american\\s*exp|\\bamex\\b).*(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b)|(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b).*(american\\s*exp|\\bamex\\b)') AND r.direction = 'debit') THEN 'charge_card_repayment'
      WHEN ((REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '(hsbc\\s*bnk|lloyds\\s*bank\\s*platin|barclaycard|\\bbcard\\b|\\bmbna\\b|capital\\s*one|\\baqua\\b|vanquis|acc-?nwest|natwest.*(?:plat|card)|halifax.*(?:card|clarity|mastercard)|tesco\\s*bank.*card|m&s\\s*bank|john\\s*lewis.*(?:card|partnership)|virgin\\s*money.*card|newday|\\bmarbles\\b|fluid\\s*card|zopa\\s*card|credit\\s*card).*(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b)|(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b).*(hsbc\\s*bnk|lloyds\\s*bank\\s*platin|barclaycard|\\bbcard\\b|\\bmbna\\b|capital\\s*one|\\baqua\\b|vanquis|acc-?nwest|natwest.*(?:plat|card)|halifax.*(?:card|clarity|mastercard)|tesco\\s*bank.*card|m&s\\s*bank|john\\s*lewis.*(?:card|partnership)|virgin\\s*money.*card|newday|\\bmarbles\\b|fluid\\s*card|zopa\\s*card|credit\\s*card)') AND NOT REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'american\\s*exp|\\bamex\\b')) AND r.direction = 'debit') THEN 'credit_card_repayment'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'unarranged\\s+overdraft|unauthori[sz]ed\\s+overdraft|unplanned\\s+overdraft|overdraft\\s+usage\\s+fee') AND r.direction = 'debit') THEN 'overdraft_unarranged'
      WHEN ((REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\boverdraft\\s+interest') AND NOT REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'arranged|unarranged|unauthori[sz]ed')) AND r.direction = 'debit') THEN 'interest_charged'
      WHEN ((REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'arranged\\s+overdraft|\\b[a-z]{3,9}\\s+overdraft\\s+fees?\\b|^\\s*overdraft\\s*$') AND NOT REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'unarranged|unauthori[sz]ed|unplanned')) AND r.direction = 'debit') THEN 'overdraft_arranged'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '^\\s*daily\\s+od\\s+int\\b') AND r.direction = 'debit') THEN 'interest_charged'
      ...
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'your\\s*plan\\s*card\\s*payment') AND r.direction = 'debit') THEN 'credit_card_repayment'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bdfh\\b|dfh\\s*financial') AND r.direction = 'debit') THEN 'debt_management_plan'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bfgfs\\b') AND r.direction = 'debit') THEN 'retail_finance_repayment'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bukcc\\b|ukcc\\.co\\.uk') AND r.direction = 'debit') THEN 'prize_competitions'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bsmart\\s*charge\\b') AND r.direction = 'debit') THEN 'fuel'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'madesimplegroup|made\\s*simple\\s*group') AND r.direction = 'debit') THEN 'business_services'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'hme\\s*rtl\\s*grp|home\\s*retail\\s*group\\s*card') AND r.direction = 'debit') THEN 'revolving_credit_repayment'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\baci\\s*uk\\b|aciukltd') AND r.direction = 'debit') THEN 'debt_collection'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'payment[\\s-]*assist') AND r.direction = 'debit') THEN 'bnpl'
    ...
    CASE
      ...
      WHEN LOWER(TRIM(r.vendor))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'repayment') THEN 'T2_compound_wik_repayment'
      WHEN LOWER(TRIM(r.vendor))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_1'
      ...
      WHEN LOWER(TRIM(r.vendor))='fitzsimons' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_2'
      ...
      WHEN LOWER(TRIM(r.vendor))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'loan\s+repayment|repayment') THEN 'T2_compound_derek_loan_repayment'
      WHEN LOWER(TRIM(r.vendor))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_3'
      ...
      WHEN LOWER(TRIM(r.vendor))='jasmine' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_4'
      ...
      WHEN LOWER(TRIM(r.vendor))='hart' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_5'
      ...
      WHEN LOWER(TRIM(r.vendor))='blackwell\'s' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_6'
      ...
      WHEN LOWER(TRIM(r.vendor))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'bibs|clothing') THEN 'T2_compound_hiba_bibs'
      WHEN LOWER(TRIM(r.vendor))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_7'
      ...
      WHEN LOWER(TRIM(r.vendor))='pier' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_8'
      ...
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '(american\\s*exp|\\bamex\\b).*(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b)|(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b).*(american\\s*exp|\\bamex\\b)') AND r.direction = 'debit') THEN 'T5_rule_R33'
      WHEN ((REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '(hsbc\\s*bnk|lloyds\\s*bank\\s*platin|barclaycard|\\bbcard\\b|\\bmbna\\b|capital\\s*one|\\baqua\\b|vanquis|acc-?nwest|natwest.*(?:plat|card)|halifax.*(?:card|clarity|mastercard)|tesco\\s*bank.*card|m&s\\s*bank|john\\s*lewis.*(?:card|partnership)|virgin\\s*money.*card|newday|\\bmarbles\\b|fluid\\s*card|zopa\\s*card|credit\\s*card).*(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b)|(\\*{3,}\\s*\\d{3,}|\\bvsa\\d{3,}|\\bpb\\d+\\*|\\bx{4,}\\d{3,}|\\b\\d{12,19}\\b|\\bddr\\b|\\bcard\\s*(?:payment|pymt|pay)\\b).*(hsbc\\s*bnk|lloyds\\s*bank\\s*platin|barclaycard|\\bbcard\\b|\\bmbna\\b|capital\\s*one|\\baqua\\b|vanquis|acc-?nwest|natwest.*(?:plat|card)|halifax.*(?:card|clarity|mastercard)|tesco\\s*bank.*card|m&s\\s*bank|john\\s*lewis.*(?:card|partnership)|virgin\\s*money.*card|newday|\\bmarbles\\b|fluid\\s*card|zopa\\s*card|credit\\s*card)') AND NOT REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'american\\s*exp|\\bamex\\b')) AND r.direction = 'debit') THEN 'T5_rule_R34'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'unarranged\\s+overdraft|unauthori[sz]ed\\s+overdraft|unplanned\\s+overdraft|overdraft\\s+usage\\s+fee') AND r.direction = 'debit') THEN 'T5_rule_R35'
      WHEN ((REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\boverdraft\\s+interest') AND NOT REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'arranged|unarranged|unauthori[sz]ed')) AND r.direction = 'debit') THEN 'T5_rule_R36'
      WHEN ((REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'arranged\\s+overdraft|\\b[a-z]{3,9}\\s+overdraft\\s+fees?\\b|^\\s*overdraft\\s*$') AND NOT REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'unarranged|unauthori[sz]ed|unplanned')) AND r.direction = 'debit') THEN 'T5_rule_R37'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '^\\s*daily\\s+od\\s+int\\b') AND r.direction = 'debit') THEN 'T5_rule_R56'
      ...
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'your\\s*plan\\s*card\\s*payment') AND r.direction = 'debit') THEN 'T5_rule_R38'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bdfh\\b|dfh\\s*financial') AND r.direction = 'debit') THEN 'T5_rule_R39'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bfgfs\\b') AND r.direction = 'debit') THEN 'T5_rule_R40'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bukcc\\b|ukcc\\.co\\.uk') AND r.direction = 'debit') THEN 'T5_rule_R41'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\bsmart\\s*charge\\b') AND r.direction = 'debit') THEN 'T5_rule_R44'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'madesimplegroup|made\\s*simple\\s*group') AND r.direction = 'debit') THEN 'T5_rule_R45'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'hme\\s*rtl\\s*grp|home\\s*retail\\s*group\\s*card') AND r.direction = 'debit') THEN 'T5_rule_R46'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), '\\baci\\s*uk\\b|aciukltd') AND r.direction = 'debit') THEN 'T5_rule_R47'
      WHEN (REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), 'payment[\\s-]*assist') AND r.direction = 'debit') THEN 'T5_rule_R48'
...
plaid_resolved AS (
  SELECT r.*,
    CASE
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'repayment') THEN 'loan_repayment_manual'
      WHEN LOWER(TRIM(r.merchant_raw))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='fitzsimons' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'loan\s+repayment|repayment') THEN 'loan_repayment_manual'
      WHEN LOWER(TRIM(r.merchant_raw))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='jasmine' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='hart' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='blackwell\'s' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'bibs|clothing') THEN 'clothing_general'
      WHEN LOWER(TRIM(r.merchant_raw))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='pier' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'transfer_p2p'
    ...
    CASE
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'repayment') THEN 'T2_compound_wik_repayment'
      WHEN LOWER(TRIM(r.merchant_raw))='wik' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_1'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='fitzsimons' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_2'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'loan\s+repayment|repayment') THEN 'T2_compound_derek_loan_repayment'
      WHEN LOWER(TRIM(r.merchant_raw))='derek' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_3'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='jasmine' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_4'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='hart' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_5'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='blackwell\'s' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_6'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'bibs|clothing') THEN 'T2_compound_hiba_bibs'
      WHEN LOWER(TRIM(r.merchant_raw))='hiba' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_7'
      ...
      WHEN LOWER(TRIM(r.merchant_raw))='pier' AND REGEXP_CONTAINS(LOWER(COALESCE(r.description_raw, '')), r'sortcode|faster\s*payment|\bfps\b|\bmob\b|\bft\b|\bbgc\b|\bbank\s+transfer\b|\btransfer\b|\d{10}') THEN 'T2_compound_p2p_payee_collision_8'
