...
9. **Build targeted training data (not just eval) for the risk leaves the retrained classifier still fails** — `cash_advance` (0 training examples), `charge_card_repayment` (1), `financial_services_other` (5), and separately `gambling_unspecified` (104 examples but still 100% wrong, likely needs either more diverse examples or a confidence-gated runtime policy rather than more raw volume). See §12 and `data/classifier_v3_retrain_report.md`. Not yet done.
