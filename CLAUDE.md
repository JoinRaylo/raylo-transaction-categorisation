...
8. **Human review pending on two new gold sets (see §12)**: `outputs/gold_risk_sample.csv` (711 rows, stratified across the 34 high-risk credit/gambling leaves, LLM-drafted) and `outputs/gold_v5_locked_sample.csv` (1,100 rows, the locked test set, LLM-drafted). Both need the same `sheet` → human review → `apply` steps as every prior gold set. Not yet done.
